...
2.  *Strong sequential exchangeability.* All the common causes of $A_t$ and $(L_s, A_s, Y)$ are measured and contained in $H_t$ for all $s \in \{t+1, ..., \tau\}$.
...
Let $\tau = 2$ and $\dd$ be an MTP that only depends on the current natural value of treatment. Suppose data are generated under this NPSEM:
$$
\begin{align}
    L_1 &= f_{L,1}(U_{L,1})\\
...
\end{align}
$$
The causal quantity of interest is $\theta = \E[Y(\bar{A}^\dd)]$
, where $Y(\bar{A}^\dd)$ is defined as
$$
\begin{aligned}
    L_1 &= f_{L,1}(U_{L,1})\\
    A_1 &= f_{A,1}(L_1, U_{A,1})\\
...
\end{align}
$$
By consistency, when we condition on $A_1=\dd(A_1)$, $L_2(\dd(A_1))$ and $A_2(\dd(A_1))$ are equal to $L_2$ and $A_2$, respectively:
$$
...
$$
Again, assuming strong sequential exchangeability $U_Y \perp A_2 \mid L_2,A_1,L_1$, therefore we can replace $A_2$ with $A_2 =\dd(A_2)$ in the innermost conditioning set:
$$
\E\big[\E\big[\E[f_Y(\dd(A_2),L_2,\dd(A_1),L_1,U_Y)\mid A_2=\dd(A_2),L_2,A_1=\dd(A_1),L_1]\,\big|\,A_1=\dd(A_1),L_1\big]\big].
$$
Finally, by one last consistency argument:
$$
\begin{align}
