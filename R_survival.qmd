...
DT::datatable(intubation, class = "compact", 
                options = list(
                  searching = FALSE, 
                  lengthChange = FALSE, 
                  ordering = FALSE, 
...
lmtp_sdr(
  ...
  time_vary = timevary_confounders,
  cens = discharge,
  shift = d, 
  mtp = TRUE,
...
In the context of survival analysis, competing risks refer to events that preclude the occurrence of the primary event of interest. In the previous examples using the `intubation` dataset, we treated hospital discharge or transfer as censoring events. These events, however, are actually competing risks because we know that if a patients was discharged or transferred out of the ICU on day $t$ they didn't die on that day. 
...
In the presence of competing risks, `lmtp` can estimate the cumulative incidence effects which may be interpreted as the _total effect_ of treatment operating through pathways that include the competing events, but not eliminating the competing events. Let's re-evaluate the effect of a delaying intubation 1-day on 14-day mortality treating discharge or transfer as competing risk. We first need to modify the data:
...
We can now re-estimate the effect of delaying intubation by 1-day. Instead of passing the `discharge` vector to the `cens` argument, we pass it to the `compete` argument.
