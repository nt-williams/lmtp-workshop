...
With estimates of $\E(Y^{A^{\dd_1}})$ and $\E(Y^{A^{\dd_0}})$ we can now calculate causal contrasts. First, let's estimate the average treatment effect (ATE). To do so, we'll use the function `lmtp_contrast()`.
...
Instead of using `lmtp_contrast` we can simply apply standard arithmetic operators to the _lmtp_ `estimate` object. On the backend, _lmtp_ uses the _ife_ package which performs automatic differentation for calculating standard errors. 
