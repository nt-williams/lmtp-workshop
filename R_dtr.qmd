...
filters:
  - webr
webr:
  packages: ['lmtp', 'superlearner', 'DynTxRegime', 'gam']
...
DT::datatable(bmiData, class = "compact", 
                options = list(
                  searching = FALSE, 
                  lengthChange = FALSE, 
                  ordering = FALSE, 
...
```{r echo=FALSE}
gtsummary::tbl_strata(bmiData, strata = A1, 
                      .tbl_fun = \(x) gtsummary::tbl_summary(x, by = A2) |> 
                        gtsummary::modify_header(
                          gtsummary::all_stat_cols() ~ "**Stage 2, {level}**"), 
                      .header = "**Stage 1, {strata}**")
```
