...
::: {.callout-caution appearance="simple"}
Be careful not to confuse doubly robust consistency with _rate double robustness_. Doubly robust consistency refers to the properties we just described. Rate double robustness refers to the property where the error of an estimator is the product of nuisance functions. 
:::
...
Let's apply the TMLE estimator using the estimated density ratios we calculated in the previous example. We first compute the weights as the cumulative product of the density ratios.

```{webr-r}
weights <- t(apply(density_ratios, 1, cumprod))
head(round(weights, 4))
```
...
for (t in 2:1) {
  ...
  # Perform the initial regression and generate predictions
  parents <- unlist(lapply(1:t, \(x) paste0(c("L", "A"), x)))
  formula <- as.formula(paste("outcome ~", paste(parents, collapse = "+")))
  initial_fit <- glm(formula, data = foo)
  initial_predictions <- predict(initial_fit, foo)

  # Perform the targeting step
  targeting_fit <- glm(outcome ~ offset(initial_predictions), weights = weights[, t])

  # Update predictions
  predictions_natural[, t] <- initial_predictions + coef(targeting_fit)
  initial_predictions_shifted <- predict(initial_fit, shifted)
  predictions_shifted[, t] <- initial_predictions_shifted + coef(targeting_fit)
  
  # Update the outcome for the next iteration
  outcome <- predictions_shifted[, t]
}
```

The estimate is then the sample average of the predicted values from the last iteration under the shifted treatment.
...
phi_3 <- foo$Y
predictions_shifted[, 3] <- phi_3

# Regress the pseudo outcome on the observed data
fit_2 <- glm(phi_3 ~ A2 + L2 + A1 + L1, data = foo)

# Generate the predictions for t = 2
predictions_natural[, 2] <- predict(fit_2)
predictions_shifted[, 2] <- predict(fit_2, dplyr::mutate(foo, A2 = 1))

# Iterate, setting t - 1 = 1
# Compute the pseudo outcome using the efficient influence function
phi_2 <- density_ratios[, 2]*(phi_3 - predictions_natural[, 2]) + predictions_shifted[, 2]

# Regress the pseudo outcome on the observed data
fit_1 <- glm(phi_2 ~ A1 + L1, data = foo)

# Generate the predictions for t = 1
predictions_natural[, 1] <- predict(fit_1)
predictions_shifted[, 1] <- predict(fit_1, dplyr::mutate(foo, A1 = 1))
```
